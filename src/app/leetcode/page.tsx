...
const LeetCodePage: React.FC = () => {
  const [stats, setStats] = useState<MatchedUser | null>(null);
  ...
  const pieChartData = stats?.submitStatsGlobal?.acSubmissionNum?.map(
  ...
  const lineChartData = stats?.submitStatsGlobal?.acSubmissionNum?.map(
    (item: SubmissionStats) => ({
      name: item.difficulty,
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white dark:bg-gray-100 p-8 rounded-lg shadow-2xl transform transition-all hover:scale-105 hover:shadow-3xl hover:rotate-1 ease-in-out duration-300">
                ...
                <div className="space-y-4">
                  {stats.submitStatsGlobal?.acSubmissionNum?.map(
                    (item: SubmissionStats) => (
                      <div
                        key={item.difficulty}
                      ...
                      >
                        <p className="text-lg">
                          <strong>{item.difficulty}:</strong> {item.count || "0"}{" "}
                          problems solved
                        </p>
                      </div>
                    )
              ...
              <div className="bg-white dark:bg-gray-100 p-8 rounded-lg shadow-2xl transform transition-all hover:scale-105 hover:shadow-3xl hover:rotate-1 ease-in-out duration-300">
                ...
                <p className="text-lg text-neutral-600 dark:text-black">
                  <strong>Across All Difficulties:</strong>{" "}
                  {stats.submitStatsGlobal?.acSubmissionNum?.reduce(
                    (total, item: SubmissionStats) => total + (item.count || 0),
                    0
                  )}{" "}
                  submissions
                </p>
